<template>
  <div class="p-6">
      ...
      <BaseTable :columns="columns" :rows="proveedoresFiltrados" :searchable="false" :loading="loading"
        ...
        <template #acciones="{ row }">
          <div class="flex items-center gap-3">
            <button @click.stop="editProveedor(row)" class="text-blue-600 hover:text-blue-800 p-1 transition-colors"
              title="Editar">
            ...
            <button @click.stop="confirmDelete(row)" class="text-red-600 hover:text-red-800 p-1 transition-colors"
              title="Eliminar">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <BaseModal :isOpen="deleteDialog" @close="deleteDialog = false" title="¿Eliminar este proveedor?">
      <div class="p-4">
        <p class="text-gray-700">Esta acción eliminará a <b>{{ proveedorToDelete?.nombre }}</b>. Los datos no podrán
          recuperarse.</p>
      </div>
      <template #footer>
        <div class="flex gap-3 justify-end w-full border-t border-gray-100 p-4">
