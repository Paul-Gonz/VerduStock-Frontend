<template>
  <div class="p-6">
    ...
    >
      <div class="p-4">
        <form @submit.prevent="saveProveedor" class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-1">
            ...
            <input v-model="form.telefono" type="text" class="border border-gray-300 rounded-lg px-3 py-2 outline-none focus:border-green-500" />
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-gray-700">Dirección</label>     
            <input v-model="form.direccion" type="text" class="border border-gray-300 rounded-lg px-3 py-2 outline-none focus:border-green-500" />
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-gray-700">Notas adicionales</label>     
            <textarea v-model="form.detalle" rows="3" class="border border-gray-300 rounded-lg px-3 py-2 outline-none focus:border-green-500"></textarea>
          </div>
        </form>
      </div>
      ...
      <template #footer>
        <div class="flex gap-3 justify-end w-full border-t border-gray-100 p-4">
          ...
          <button @click="saveProveedor" :disabled="saving" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
    ...
    >
      <div class="p-4">
        <p class="text-gray-700">Esta acción eliminará a <b>{{ proveedorToDelete?.nombre }}</b>. Los datos no podrán recuperarse.</p>
      </div>
      <template #footer>
        <div class="flex gap-3 justify-end w-full border-t border-gray-100 p-4">
          ...
          <button @click="deleteProveedor" :disabled="deleting" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
