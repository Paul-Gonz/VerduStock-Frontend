        ...
        <div class="max-w-7xl mx-auto flex flex-col gap-6">
            ...
            <section class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <article class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <client-only>
                        <template v-if="donutSeries && donutSeries.length > 0">
                            <component :is="Apexchart" v-if="Apexchart" type="donut" height="260" :options="donutOptions"
                                :series="donutSeries" />
                        </template>
                        <div v-else class="flex flex-col items-center justify-center h-[260px] bg-green-50/50 rounded-xl border border-dashed border-green-200">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 text-green-400 mb-3">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6a7.5 7.5 0 1 0 7.5 7.5h-7.5V6Z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0 0 13.5 3v7.5Z" />
                            </svg>
                            <p class="text-sm font-medium text-green-800">Aún no hay datos para mostrar</p>
                            <p class="text-xs text-green-600/70 mt-1">Registra productos para visualizar esta gráfica</p>
                        </div>
                    </client-only>
                </article>
                <article class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <client-only>
                        <template v-if="barSeries && barSeries.length > 0 && barSeries[0]?.data?.length > 0">
                        ...
                        <div v-else class="flex flex-col items-center justify-center h-[260px] bg-green-50/50 rounded-xl border border-dashed border-green-200">
                            ...
                            <p class="text-sm font-medium text-green-800">Aún no hay datos para mostrar</p>
                            <p class="text-xs text-green-600/70 mt-1">Registra productos para visualizar la inversión</p>
                        </div>
                    </client-only>
                </article>
            </section>
