    ...
    >
      ...
      <div class="flex items-start justify-between gap-4 border-b border-gray-100 px-6 py-5">
        <div class="min-w-0">
          ...
          <h2 class="truncate text-xl font-bold text-gray-900">{{ product.nombre }}</h2>
        ...
        <button
          @click="$emit('close')"
          class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
        >
      ...
      <div class="flex flex-1 flex-col gap-6 overflow-y-auto p-6">
        ...
        <section class="flex flex-col gap-3">
          <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400">Stock</h3>
          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-xl bg-gray-50 p-4">
              <p class="mb-1 text-xs text-gray-400">Stock Actual</p>
              <p class="text-2xl font-bold text-gray-800">
                {{ stockActual }} <span class="text-sm font-medium text-gray-400">kg</span>
              </p>
            </div>
            <div class="rounded-xl bg-gray-50 p-4">
              <p class="mb-1 text-xs text-gray-400">Stock Inicial (neto)</p>
              <p class="text-2xl font-bold text-gray-800">
                {{ stockInicial }} <span class="text-sm font-medium text-gray-400">kg</span>
              </p>
            </div>
            <div class="rounded-xl bg-orange-50 p-4">
              <p class="mb-1 text-xs text-orange-400">Desperdicio</p>
              <p class="text-2xl font-bold text-orange-600">
                {{ desperdicio }} <span class="text-sm font-medium text-orange-300">kg</span>
              </p>
            </div>
            <div class="rounded-xl bg-gray-50 p-4">
              <p class="mb-1 text-xs text-gray-400">Vencimiento</p>
        ...
        <!-- Sección: Precios -->
        <section class="flex flex-col gap-3">
          <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400">Precios</h3>
          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-xl bg-gray-50 p-4">
              <p class="mb-1 text-xs text-gray-400">Precio Compra / kg</p>
              <p class="text-xl font-bold text-gray-800">{{ fmt(precioCompra) }}</p>
            </div>
            <div class="rounded-xl bg-green-50 p-4">
              <p class="mb-1 text-xs text-green-500">Precio Venta / kg</p>
              <p class="text-xl font-bold text-green-700">{{ fmt(precioVenta) }}</p>
            </div>
          </div>
          <div class="rounded-xl bg-gray-50 p-4">
            <p class="mb-1 text-xs text-gray-400">Proveedor</p>
            <p class="text-sm font-semibold text-gray-700">{{ product.proveedor }}</p>
          </div>
        </section>

        <!-- Sección: Rentabilidad -->
        <section class="flex flex-col gap-4">
          <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400">Rentabilidad</h3>
          ...
          <div class="flex items-center justify-center py-2">
            <div class="relative h-36 w-36">
              ...
              <div class="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span class="text-xs font-medium text-gray-400">margen</span>
              </div>
            </div>
          </div>

          <!-- Desglose financiero -->
          <div class="flex flex-col gap-3 rounded-xl bg-gray-50 p-4">
            <div class="flex justify-between text-sm">
              <span class="text-gray-500">Costo total</span>
              <span class="font-semibold text-gray-700">{{ fmt(costoTotal) }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-gray-500">Ganancia potencial</span>
              <span class="font-semibold text-green-600">{{ fmt(gananciaTotal) }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-gray-500">Valor total en venta</span>
              <span class="font-bold text-gray-800">{{ fmt(valorVentaTotal) }}</span>
            </div>

            <!-- Barra apilada costo vs ganancia -->
            <div>
              <div class="h-3 overflow-hidden rounded-full bg-gray-200 flex">
                <div
            ...
            <div class="flex justify-between border-t border-gray-200 pt-3 text-sm">
              <span class="text-gray-500">Ganancia por kg</span>
              <span class="font-semibold" :class="gananciaPorKg >= 0 ? 'text-green-600' : 'text-red-500'">
                {{ fmt(gananciaPorKg) }}
              </span>
            </div>
          </div>
...
const margenTextColor = computed(() => {
  if (margenPct.value >= 30) return 'text-green-600'
  if (margenPct.value >= 10) return 'text-amber-500'
  return 'text-red-500'
})
...
const badgeClass = computed(() => ({
  'bg-green-100 text-green-700':   estado.value === 'ok',
  'bg-amber-100 text-amber-700':   estado.value === 'bajo',
  'bg-orange-100 text-orange-700': estado.value === 'por_vencer',
  'bg-red-100 text-red-700':       estado.value === 'vencido',
}))
...
const vencimientoClass = computed(() => {
  ...
  if (exp.getTime() - now.getTime() <= 7 * 86400000) return 'text-amber-600'
